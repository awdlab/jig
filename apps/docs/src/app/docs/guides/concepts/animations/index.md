...
Every control starts with an `ngn-control-initializing` class that keeps it
...
```ts
provideNgnControls({ theme: { preset: novaCoral }, disableAnimations: true });
```
...
```ts
provideNgnControls({ theme: { preset: novaCoral }, respectReducedMotion: false });
```
...
`--ngn-anim-time-snappy-fade`). Retiming the whole UI is a token override
rather than editing keyframes:

```css
:root {
  --ngn-anim-time-snappy-fade: 120ms;
}
```
...
```css
.card {
  transition: box-shadow var(--ngn-anim-time-snappy-fade) var(--ngn-anim-ease-fade);
}
```
