...
and they live in `@ngneers/controls/api`.
...
```ts
import { transformToNgnItem, type NgnItemFields } from '@ngneers/controls/api';
...
shortcut is the host's decision: `ngn-command` registers its items page-wide,
...
What [`<ngn-action-button>`](/components/button) renders — an action item plus
