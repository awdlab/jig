...
```
--ngn-color-<family>-<shade>     e.g. --ngn-color-primary-500
```
...
```
--ngn-color-<family>-<shade>-contrast    e.g. --ngn-color-primary-500-contrast
```
...
Beyond the ramps there are a few global color tokens: `--ngn-color-background`,
`--ngn-color-text`, and `--ngn-color-border`, plus grouped `disabled` and `invalid`
