...
The bundled `awd-migrate` skill drives the workflow: inventory the source
...
<!-- after -->
<awd-select [(value)]="city" [items]="cities" />
```

**2. Field wrappers.** `mat-form-field` and PrimeNG's float-label wrappers
become [`awd-input-field`](/components/input-field), which owns the label,
...
> `<awd-input-field [inputId]="'user-email'">` — or every external
...
templates), the awd equivalents take **data**:

```html
<awd-select [items]="items" [(value)]="value" />
```
...
- **CSS.** Theme CSS lives in the `awd-controls` cascade layer, so the other
