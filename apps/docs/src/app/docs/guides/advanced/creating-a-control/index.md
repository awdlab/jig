...
@Component({
  selector: 'awd-widget', // awd- prefix, kebab === folder name
...
- **Selector:** `awd-{name}` element, or a camelCase attribute (`ngnButton`, `ngnInput`)
...
- **Directive input aliases:** `awd{Directive}{Prop}`, where the alias suffix equals the
