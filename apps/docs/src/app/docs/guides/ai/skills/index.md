Alongside the [MCP Server](/guides/mcp), the `@ngneers/controls-mcp` package
...
| ------------------- | ---------------------------------------------------------------------------------------------- |
| `ngn-controls`      | Discover controls, look up the real API via `get_control`, honor signal conventions.           |
| `ngn-migrate`       | Inventory `p-*` / `mat-*` / `ejs-*` usage, map each to its ngn target, rewrite, and flag gaps. |
| `ngn-theme`         | Author a product theme with tokens only — schema → anatomy → `scaffold_theme_part`.            |
| `ngn-build-feature` | Compose controls into a feature: `recommend_controls` → recipe → `get_control` → scaffold.     |
...
> spec like `@ngneers/controls-mcp` is not itself a source, so install from the
