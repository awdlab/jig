`ngn-defer` delays creating a piece of UI until it is first shown, and
...
<ngn-defer [open]="open()" [lazyContent]="body" />
...
Projected content is created by the **parent** component, so `ngn-defer` never
...
```html
<ngn-defer [open]="open()" [lazyContent]="row" [lazyContentContext]="{ $implicit: user() }" />
...
| Skip constructing a subtree until it is shown     | `ngn-defer` with `lazyContent`                      |
| Simply add/remove a subtree                       | `@if`                                               |
| Keep an expensive subtree alive across open/close | `ngn-defer` with `cache`                            |
