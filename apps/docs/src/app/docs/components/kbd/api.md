...
All exported from `@ngneers/controls/kbd`. These are the pure, Angular-free
building blocks `ngn-kbd` and `[ngnKeyboardShortcut]` are built on — reach for
...
| `closestShortcutScope(element: Element \| null): NgnKeyboardShortcut \| null` | The nearest `[ngnKeyboardShortcut]` scope at or above `element`, found by walking up the DOM. This is how `NgnActionButton` finds its scope, and how you'd wire up a custom registering control. |

The platform check behind `mod` lives with the other platform helpers:
`isMacPlatform()` from `@ngneers/controls/api/ng` returns whether the current
...
| `NgnShortcutBinding` | `{ shortcut, callback, disabled?, global? }` — the shape passed to `[ngnKeyboardShortcut]` and to `NgnKeyboardShortcut.register()`. `global` drops the focus requirement for that one binding. |
