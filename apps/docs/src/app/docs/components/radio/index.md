The Radio group lets the user pick a single option from a set. Compose `ngn-radio`
options inside an `ngn-radio-group`; the group (`role="radiogroup"`) owns the
selected value via a two-way `value`, while each `ngn-radio` contributes its
...
Wrap `ngn-radio` options in an `ngn-radio-group` and bind `value` with
...
A disabled `ngn-radio` is skipped during keyboard navigation and can't be
