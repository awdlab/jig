The OTP control (`ngn-otp`) collects a short verification / one-time-password
code across a row of single-character cells. It is a self-contained value
control — bind `[(value)]` directly on `<ngn-otp>`. The value is the composed
...
Being a value control, `ngn-otp` supports the shared `invalid` state. Here the
