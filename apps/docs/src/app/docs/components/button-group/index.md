The Button Group component (`awd-button-group`) collects a set of related
...
`button[ngnButton]` elements, or `awd-toggle-button`s when you want pressable
buttons; the group adapts to either. Style variants (`kind`, `color`) live on
the individual buttons, not on the group.

### Basic Usage

Three related `button[ngnButton]`s wrapped in `<awd-button-group>` render as one
...
Placing `awd-toggle-button`s inside the group gives each button an independent
