...
Pair the input with `jig-spin-buttons` for clickable increment/decrement
controls. Inside an `jig-input-field` the buttons find the number input
...
- **Stacked** (default): `<jig-spin-buttons />` — up/down chevrons at the trailing edge.
- **Inline**: `<jig-spin-buttons kind="inline" />` — both buttons side by side.
