Display content in a tabbed interface. A `<ngn-tabs>` wraps a set of
`<ngn-tab>` children, each identified by a required `tabId` and carrying two
...
Each `<ngn-tab>` supplies its label through a `#header` template and its panel
through a `#content` template; `<ngn-tabs>` renders the header strip and the
...
Tabs are projected content, so adding or removing `<ngn-tab>` elements at runtime
