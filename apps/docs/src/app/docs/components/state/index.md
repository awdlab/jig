The State component (`<awd-state>`) is a compact status indicator for one of
...
Drop an `<awd-state>` inside an `ngnButton` to show progress in place. By default
...
Placed inside an `awd-input-field`, the indicator acts as a trailing status
