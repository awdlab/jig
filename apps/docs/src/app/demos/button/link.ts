...
@Component({
  ...
  template: `
    ...
    <p class="max-w-prose" (click)="$event.preventDefault()">
      ...
      the paragraph, so
      <a jigButton kind="link" href="#/components/button">a navigating anchor</a> and
      <button jigButton kind="link" (click)="ran = true">an in-place action</button> both read as
      ...
      <a jigButton kind="link" href="#/components/icon"
      ...
      row, like
      <a jigButton kind="link" href="#/components/button">this deliberately verbose link label</a>
      does. A disabled link stays inline too:
      <button jigButton kind="link" disabled>unavailable action</button>.
    </p>
  `,
})
