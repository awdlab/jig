...
export function flattenTree<T, V>(
  items: readonly NgnTreeItem<T, V>[],
...
export function applyLoadedChildren<T, V>(
  items: readonly NgnTreeItem<T, V>[],
  loaded: ReadonlyMap<V, readonly NgnTreeItem<T, V>[]>
): readonly NgnTreeItem<T, V>[] {
  if (loaded.size === 0) {
    return items;
  }
...
export function collectLeafValues<T, V>(item: NgnTreeItem<T, V>, parentDisabled = false): V[] {
...
export function collectBranchValues<T, V>(items: readonly NgnTreeItem<T, V>[]): V[] {
