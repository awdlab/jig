...
import type { DepClass, AwdPassthrough } from '@awdlab/jig/base';
 ...
 * public `[pt]` input (`AwdPassthrough`) and the internal `[ptClass]` /
...
describe('passthrough typing', () => {
  describe('public API — AwdPassthrough (the [pt] input)', () => {
    it('accepts own scope classes and per-instance dependency slots', () => {
      assertType<AwdPassthrough<'calendar'>>({
    ...
    it('excludes projected slots from [pt]', () => {
      ...
      assertType<AwdPassthrough<'inputField'>>({ input: { root: {} } });
    });
  });
