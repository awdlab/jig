...
import type { EnumField, Field, MaskInputCfg, NumberField, Part } from './types';

export type RenderToken =
  | { kind: 'sep'; text: string }
  | { kind: 'section'; ord: number; text: string; placeholder: boolean };

export function resolveMask(input: MaskInputCfg | string | null): Part[] | null {
  if (!input) return null;
  const cfg: MaskInputCfg | undefined =
    typeof input === 'string'
      ? ((MASKS as Record<string, MaskInputCfg>)[input] ??
        (DATE_TIME_MASKS as Record<string, MaskInputCfg>)[input])
      : input;
