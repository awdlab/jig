...
import type { MaskInputCfg, NumberSegment, EnumSegment, MaskSegment } from './types';

function findSegment(mask: MaskInputCfg, segmentName: string): MaskSegment | undefined {
  return mask.find(
    entry => typeof entry === 'object' && 'segment' in entry && entry.segment === segmentName
  ) as MaskSegment | undefined;
}

function separators(mask: MaskInputCfg): string[] {
