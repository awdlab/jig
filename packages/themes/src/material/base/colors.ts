...
// Contrast-safe foreground (text/icon) shade of the palette for use on the page background.
...
export const colorsTemplate = createVariableTemplate({
  ...
  variables: {
    ...repeatVariables(
      ...
      { ...shadeVariables, ...contrastVariables, ...tonalVariables, ...foregroundVariable }
    ),
    background: null,
    border: null,
    text: null,
...
// Shade order for the foreground pick: least-dark first, so the chosen text
// color stays as vibrant as possible while still clearing AA.
const FOREGROUND_SHADES = ['500', '600', '700', '800', '900', '950'] as const;

/**
 * A contrast-safe foreground (text/icon) shade of a palette for use ON the page
 * background — the LEAST-dark shade that still clears AA against it. This lets a
 * light-hued primary (teal, amber, …) render as readable primary-colored text
 * (it darkens to e.g. 700/800) while a dark primary (indigo) keeps its vibrant
 * 500. Mode-aware: `shades` is already mode-reversed, so in dark mode it resolves
 * to a light shade against the dark background. Emitted as a ref to the winner.
 */
function foregroundRef(name: string, shades: Record<string, RGB>, backgroundRgb: RGB): string {
  for (const shade of FOREGROUND_SHADES) {
    const rgb = shades[shade];
    if (!rgb) {
      continue;
    }
    const match = bestContrast(backgroundRgb, [{ key: 'x', rgb }]);
    if (match && match.ratio >= CONTRAST_AA) {
...
function getThemeColors(
  isDark: boolean,
  primaryShades: Record<string, string> = indigoColor,
  primaryRgb: Record<string, RGB> = indigoColorRgb,
  neutralShades: typeof neutralColor = neutralColor,
  neutralRgb: typeof neutralColorRgb = neutralColorRgb
) {
  ...
  const palette = (
    ...
    return {
      ...
      foreground: foregroundRef(name, rgb, backgroundRgb),
    };
  };

  return {
    primary: palette('primary', primaryShades, primaryRgb),
    secondary: palette('secondary', neutralShades, neutralRgb),
    accent: palette('accent', pinkColor, pinkColorRgb),
    error: palette('error', redColor, redColorRgb),
    warning: palette('warning', orangeColor, orangeColorRgb),
    info: palette('info', blueColor, blueColorRgb),
    success: palette('success', greenColor, greenColorRgb),
    ...
    invalid: {
      text: p(redColor)['500'],
      border: p(redColor)['400'],
      background: p(redColor)['50'],
    },
  };
}

// ponytail: getThemeColors / contrastRefs / tonalRefs are duplicated from nova/base/colors.ts
// (the repo's "each theme owns its colors.ts" convention, same as shade). Upgrade path =
// extract a shared buildThemeColors(palettes, isDark) if a 4th theme lands.
export const material = createThemePart({
  scope: 'color',
...
/**
 * Full material color-token values for a given primary base color — the same output
 * that backs the built-in {@link material} part, but with the primary palette
 ...
 */
export function materialColorValues(
  primaryHex: string | null,
  isDark: boolean,
  surfaceHex: string | null = null
) {
  const primaryShades = primaryHex == null ? indigoColor : getColorPalette(primaryHex);
  const primaryRgb = primaryHex == null ? indigoColorRgb : getColorPaletteRgb(primaryHex);
  const neutralShades = surfaceHex == null ? neutralColor : getColorPalette(surfaceHex);
  const neutralRgb = surfaceHex == null ? neutralColorRgb : getColorPaletteRgb(surfaceHex);
...
type ThemePaletteVarName =
  ...
  | `color.${ThemePaletteColor}.foreground`;
  ...
  return (
    ...
    .map(
      ...
      ${c(`color-${color}`)} {
        ...
        --theme-color-foreground: ${v(`color.${color}.foreground`)};
      }
      `
    )
    .join('\n');
