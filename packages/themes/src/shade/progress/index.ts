...
export const progressStyles = createThemePart({
  ...
  root: {
    css: ({ v, c }) => css`
      ${c('track')} {
        background: color-mix(in srgb, ${v('color.primary.base')} 20%, transparent);
      ...
      ${c('circular')} ${c('track')} {
        stroke: color-mix(in srgb, ${v('color.primary.base')} 20%, transparent);
        background: none;
      }
      ${c('circular')} ${c('fill')} {
        stroke: ${v('color.primary.base')};
        background: none;
