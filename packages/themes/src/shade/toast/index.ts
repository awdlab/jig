...
export const toastStyles = createThemePart({
  ...
  root: {
    // Popover-surfaced card by default; a color-* class tints it (e.g. destructive toasts).
    css: ({ v, c }) => css`
      ${slotColors(c, v)}
      ...
      ${c('root')} {
        background: var(--theme-bg, ${v('color.popover.base')});
        color: var(--theme-fg, ${v('color.popover.foreground')});
