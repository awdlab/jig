  ...
  root: {
    css: ({ c }) => css`
      ${c('root')} {
        ...
        z-index: 1;
        /* translate() is physical, so the corner overhang must be negated in RTL to keep
           pointing away from the anchor rather than back into it. */
        --overhang: 1;
      }
      ${c('root')}:dir(rtl) {
        --overhang: -1;
      ...
      ${c('top-end')} {
        top: 0;
        inset-inline-end: 0;
        transform: translate(calc(50% * var(--overhang)), -50%);
      }
      ${c('top-start')} {
        top: 0;
