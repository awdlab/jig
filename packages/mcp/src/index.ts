...
async function serve(): Promise<void> {
  ...
  console.error('[@ngneers/controls-mcp] ready (stdio)');
}
...
if (command === 'init') {
  runInit(process.argv.slice(3))
    .then(() => process.exit(0))
    .catch(err => {
      console.error('[@ngneers/controls-mcp] init failed:', err);
      process.exit(1);
    });
} else if (command === '--help' || command === '-h') {
  console.log(
    'Usage:\n' +
      '  ngneers-controls-mcp            Start the MCP server (stdio).\n' +
      '  ngneers-controls-mcp init       Install the bundled agent skills into ./.claude/skills.\n' +
  ...
  serve().catch(err => {
    console.error('[@ngneers/controls-mcp] fatal:', err);
    process.exit(1);
  });
}
