...
export function registerFeature(server: McpServer, pack: KnowledgePack): void {
  server.registerTool(
    'recommend_controls',
    {
      ...
      description:
        'Given a feature goal (e.g. "a filterable table with row selection"), suggest the ngn ' +
    ...
    async ({ goal }) => {
      ...
      const examples = pack.examples
        ...
        .slice(0, 6)
        .map(e => `- ${e.scenario} (\`${e.control}\`) · ngn://example/${e.slug}`);
  ...
  server.registerPrompt(
    'build_feature',
    {
      title: 'Build a feature with ngn controls',
      description: 'Guided workflow to build a new feature using @ngneers/controls.',
      ...
      messages: [
        {
          ...
          content: {
            type: 'text',
            text:
              `Help me build this with @ngneers/controls: "${goal}".\n` +
              `1. Call recommend_controls("${goal}") to pick the controls + any recipe.\n` +
              `2. Read matching recipes (ngn://recipe/<slug>) and get_control for each control ` +
              `to confirm real inputs/outputs.\n` +
              `3. Scaffold the feature using ngn conventions (signal inputs, ngn-input-field ` +
