...
**Validation messages can be scoped to a control.** A control's own `<scope>.errors.<key>` translation now wins over the shared `errors.<key>`, so `jig-otp` says "Enter the full code" and `jig-tag-input` "Add at least one tag" where every other control keeps the generic wording.
