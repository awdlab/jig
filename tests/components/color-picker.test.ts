...
import { parseColor } from '@ngneers/controls/utils';
...
test('format toggle cycles hex -> rgb -> hsl', async ({ page }) => {
  await loadComponent(
    page,
    {
      template: `<ngn-color-picker [inline]="true" [value]="'#3366cc'" />`,
      imports: ['color-picker'],
    },
    { inputs: {} }
  );
  const cp = new NgnColorPickerHarness(page.locator('ngn-color-picker'));
  const labels = page.locator('ngn-color-picker [class*="channel-label"]');
  const boxes = page.locator('ngn-color-picker [class*="channel"] input');
  ...
  await loadComponent(
    page,
    {
      template: `
        ...
        <ngn-color-picker #picker [popover]="true" [anchor]="anchor" [value]="'#ff0000'" />
      `,
      imports: ['color-picker'],
    },
    { inputs: {} }
...
test('editing an RGB channel updates the value', async ({ page }) => {
  const handle = await loadComponent(
    page,
    {
      template: `<ngn-color-picker [inline]="true" [alpha]="false" [format]="'rgb'" [value]="'#000000'" (valueChange)="output('value', $event)" />`,
      imports: ['color-picker'],
    },
    { inputs: {} }
  );
...
test('typing a valid hex in the field updates the value', async ({ page }) => {
  ...
  await page.locator('ngn-color-picker input').blur();
...
test('short hex is not expanded while typing — only on commit (regression)', async ({ page }) => {
  ...
  const input = page.locator('ngn-color-picker input');
  await input.click();
...
test('accessibility (axe): trigger mode', async ({ page }) => {
  await loadComponent(
    page,
    {
      template: `<ngn-color-picker [label]="'Pick a color'" />`,
      imports: ['color-picker'],
    },
    { inputs: {} }
  );
...
test('accessibility (axe): inline mode', async ({ page }) => {
  await loadComponent(
    page,
    {
      template: `<ngn-color-picker [inline]="true" [label]="'Pick a color'" />`,
      imports: ['color-picker'],
    },
    { inputs: {} }
  );
...
test('format toggle on an untouched picker does not emit a value', async ({ page }) => {
  const handle = await loadComponent(
    page,
    {
      template: `<ngn-color-picker [inline]="true" (valueChange)="output('value', $event)" />`,
      imports: ['color-picker'],
    },
    { inputs: {} }
  );
