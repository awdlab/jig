...
test('renders a page button per page and marks the current one', async ({ page }) => {
  ...
  const paginator = page.locator('awd-paginator');
...
test('fixedPageSize hides the page-size selector', async ({ page }) => {
  ...
  const paginator = page.locator('awd-paginator');
  await expect(paginator.locator('awd-select')).toHaveCount(0);

  // Re-enabling the selector renders the select control.
  await handle.setInputs({ fixedPageSize: false });
  await expect(paginator.locator('awd-select')).toHaveCount(1);
});

const COMPACT_TEMPLATE = `<awd-paginator
  [mode]="'compact'"
...
test('visual', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('awd-paginator [aria-current="page"]')).toHaveText('1');
