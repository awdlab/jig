...
test('progress bar', async ({ page }) => {
  ...
  const bar = page.locator('ngn-snackbar >> css=[class*="progressBar"]');
  ...
  await test.step('renders and is styled when auto-hiding', async () => {
    ...
    expect(styles.animationName).toBe('ngn-snackbar-progressBar');
  ...
  await test.step('pauses on hover and resumes on leave when pauseOnHover is set', async () => {
    ...
    const snackbar = page.locator('ngn-snackbar');
