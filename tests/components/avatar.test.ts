...
import { NgnAvatarHarness } from '@awdlab/jig-playwright';
...
test('initials & image', async ({ page }) => {
  const handle = await loadComponent(
    page,
    {
      template: `<awd-avatar [initials]="inputs().initials" [image]="inputs().image" />`,
      imports: ['avatar'],
    },
    {
      inputs: {
  ...
  const avatar = new NgnAvatarHarness(page.locator('awd-avatar'));
...
test('sizes', async ({ page }, testInfo) => {
    ...
    page,
    {
      template: `@for(size of inputs().sizes; track size) {
        @for(initial of inputs().initials; track initial) {
          <awd-avatar [initials]="initial ? initial : undefined" [image]="initial ? undefined : 'example.png'" [size]="size" />
        }
          <br />
      }`,
      imports: ['avatar'],
  ...
  await expect(page.locator('awd-avatar')).toHaveCount(45);
...
test('accessibility (axe)', async ({ page }) => {
  await loadComponent(
    page,
    {
      template: `<awd-avatar [initials]="inputs().initials" />`,
      imports: ['avatar'],
    },
    { inputs: { initials: 'AB' } }
  );
