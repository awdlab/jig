...
import { NgnSwitchHarness } from '@awdlab/jig-playwright';
...
test('base', async ({ page }, testInfo) => {
  const handle = await loadComponent(
    page,
    {
      template: `<awd-switch [value]="inputs().value" (valueChange)="output('value', $event)" />`,
      imports: ['switch'],
    },
    {
      inputs: {
  ...
  const switchControl = new NgnSwitchHarness(page.locator('awd-switch'));
...
test('accessibility (axe)', async ({ page }) => {
  await loadComponent(
    page,
    {
      ...
      template: `
        <awd-switch #sw [value]="inputs().value" />
