...
test('base', async ({ page }, testInfo) => {
  const handle = await loadComponent(
    page,
    {
      template: `<ngn-progress [value]="inputs().value" />`,
      imports: ['progress'],
    },
    {
      inputs: {
  ...
  const progress = new NgnProgressHarness(page.locator('ngn-progress'));
...
test('indeterminate mode', async ({ page }, testInfo) => {
  const handle = await loadComponent(
    page,
    {
      template: `<ngn-progress [value]="inputs().value" [indeterminate]="inputs().indeterminate" />`,
      imports: ['progress'],
    },
    {
      inputs: {
...
test('circular mode', async ({ page }, testInfo) => {
  const handle = await loadComponent(
    page,
    {
      template: `<ngn-progress [value]="inputs().value" [circular]="inputs().circular" [radius]="inputs().radius" [thickness]="inputs().thickness" />`,
      imports: ['progress'],
    },
    {
      inputs: {
...
test('circular indeterminate mode', async ({ page }, testInfo) => {
  const handle = await loadComponent(
    page,
    {
      template: `<ngn-progress [circular]="inputs().circular" [indeterminate]="inputs().indeterminate" [radius]="inputs().radius" [thickness]="inputs().thickness" />`,
      imports: ['progress'],
    },
    {
      inputs: {
