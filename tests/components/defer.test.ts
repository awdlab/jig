...
// ngn-defer gates its `lazyContent` template on `open() || (cache() && hasBeenOpened()) || !lazy()`.
...
const content = (page: Page) =>
  page.locator('ngn-defer').getByText('deferred content', { exact: true });
...
test('lazy (default): content is absent until opened, then torn down on close', async ({
  ...
  const handle = await loadComponent(
    page,
    {
      template: `
        ...
        <ngn-defer [open]="inputs().open" [lazyContent]="tpl"></ngn-defer>
      `,
      imports: ['defer'],
    },
    { inputs: { open: false } }
  ...
  await expect(page.locator('ngn-defer')).toHaveAttribute('aria-hidden', 'true');

  await handle.setInputs({ open: true });
  await expect(content(page)).toBeVisible();
  await expect(page.locator('ngn-defer')).toHaveAttribute('aria-hidden', 'false');
...
test('cache: content stays rendered after being opened once', async ({ page }) => {
  const handle = await loadComponent(
    page,
    {
      template: `
        <ng-template #tpl>deferred content</ng-template>
        <ngn-defer [open]="inputs().open" [lazyContent]="tpl" [cache]="true"></ngn-defer>
      `,
      imports: ['defer'],
    },
    { inputs: { open: false } }
...
test('lazy=false: content renders immediately regardless of open', async ({ page }) => {
  await loadComponent(page, {
    template: `
      <ng-template #tpl>deferred content</ng-template>
      <ngn-defer [open]="false" [lazy]="false" [lazyContent]="tpl"></ngn-defer>
    `,
    imports: ['defer'],
  });
