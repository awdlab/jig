...
// jig-defer gates its `lazyContent` template on `open() || (cache() && hasBeenOpened()) || !lazy()`.
...
const content = (page: Page) =>
  page.locator('jig-defer').getByText('deferred content', { exact: true });
...
test('lazy (default): content is absent until opened, then torn down on close', async ({
  ...
  const handle = await loadComponent(
    page,
    {
      template: `
        ...
        <jig-defer [open]="inputs().open" [lazyContent]="tpl"></jig-defer>
      `,
      imports: ['defer'],
    },
    { inputs: { open: false } }
  ...
  await expect(page.locator('jig-defer')).toHaveAttribute('aria-hidden', 'true');

  await handle.setInputs({ open: true });
  await expect(content(page)).toBeVisible();
  await expect(page.locator('jig-defer')).toHaveAttribute('aria-hidden', 'false');
...
test('cache: content stays rendered after being opened once', async ({ page }) => {
  const handle = await loadComponent(
    page,
    {
      template: `
        <ng-template #tpl>deferred content</ng-template>
        <jig-defer [open]="inputs().open" [lazyContent]="tpl" [cache]="true"></jig-defer>
      `,
      imports: ['defer'],
    },
    { inputs: { open: false } }
...
test('lazy=false: content renders immediately regardless of open', async ({ page }) => {
  await loadComponent(page, {
    template: `
      <ng-template #tpl>deferred content</ng-template>
      <jig-defer [open]="false" [lazy]="false" [lazyContent]="tpl"></jig-defer>
    `,
    imports: ['defer'],
  });
