...
test('linear: incomplete optional step does not block forward navigation', async ({ page }) => {
  await loadComponent(
    ...
    {
      template: `
        <ngn-stepper [linear]="true">
          <ngn-step [label]="'One'" [completed]="true"><ng-template #content>One</ng-template></ngn-step>
          <ngn-step [label]="'Two'" [optional]="true"><ng-template #content>Two</ng-template></ngn-step>
          <ngn-step [label]="'Three'"><ng-template #content>Three</ng-template></ngn-step>
        </ngn-stepper>`,
    ...
    page,
    {
      template: `
        <ngn-stepper>
          ...
          <ngn-step [label]="'C'"><ng-template #content>C</ng-template></ngn-step>
        </ngn-stepper>`,
      imports: ['stepper', 'step'],
    },
    { inputs: {} }
...
test('visual: active + completed markers', async ({ page }, testInfo) => {
  ...
  await expectScreenshot(page.locator('ngn-stepper'), testInfo);
});
