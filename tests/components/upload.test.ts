...
import { NgnUploadHarness } from '@awdlab/jig-playwright';
...
test('keyboard: native input is the focus stop and Enter opens the picker', async ({ page }) => {
  await loadComponent(page, {
    template: `<awd-upload><input type="file" multiple /></awd-upload>`,
    imports: ['upload'],
  });

  const upload = new NgnUploadHarness(page.locator('awd-upload'));
...
test('keyboard: drag-only input is not focusable', async ({ page }) => {
  await loadComponent(page, {
    template: `<awd-upload interaction="drag"><input type="file" multiple /></awd-upload>`,
    imports: ['upload'],
  });
...
test('confirm: files queue as pending until the upload button is pressed', async ({ page }) => {
  await loadComponent(page, {
    template: `<awd-upload mode="confirm" confirmTrigger="all"><input type="file" multiple /></awd-upload>`,
    imports: ['upload'],
  });
...
test('manual: no trigger is rendered; files stay pending', async ({ page }) => {
  await loadComponent(page, {
    template: `<awd-upload mode="manual"><input type="file" multiple /></awd-upload>`,
    imports: ['upload'],
  });
...
test('manual: uploadAll() resolves once files settle, with their final state', async ({ page }) => {
  ...
  const states = await page.evaluate(async () => {
    const el = document.querySelector('awd-upload')!;
...
test('accessibility (axe)', async ({ page }) => {
  ...
  await loadComponent(page, {
    template: `<awd-upload><input type="file" multiple />Drag files here or click to browse</awd-upload>`,
    imports: ['upload'],
  });
...
test('visual', async ({ page }, testInfo) => {
  await loadComponent(page, {
    template: `
      <awd-upload class="page-center" style="width: 420px; display: block;">
