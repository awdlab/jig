...
import { NgnMessageHarness } from '@awdlab/jig-playwright';
...
test('features', async ({ page }, testInfo) => {
  const handle = await loadComponent(
    page,
    {
      ...
      <awd-message
        class="page-center"
        [icon]="inputs().icon"
      >Message text content</awd-message>
    `,
      imports: ['message'],
    },
    {
  ...
  const message = new NgnMessageHarness(page.locator('awd-message'));
          ...
          @for (kind of inputs().kinds; track $index) {
            ...
            <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
              @for (color of inputs().colors; track $index) {
                <awd-message [kind]="kind" [color]="color">{{ color ?? 'default' }} message</awd-message>
              }
            </div>
          }
        </div>
