...
import { NgnEditInplaceHarness } from '@awdlab/jig-playwright';
...
test('base', async ({ page }, testInfo) => {
  const handle = await loadComponent(page, {
    template: `
      <awd-edit-inplace [value]="inputs().value" (valueChange)="output('valueChange', $event)" />
    `,
    imports: ['editInplace'],
  });

  const editInplace = new NgnEditInplaceHarness(page.locator('awd-edit-inplace'));
...
test('accessibility (axe)', async ({ page }) => {
  const handle = await loadComponent(page, {
    template: `
      <awd-edit-inplace [value]="inputs().value" [label]="'Display name'" (valueChange)="output('valueChange', $event)" />
    `,
    imports: ['editInplace'],
  });
