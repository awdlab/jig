...
test('half-step: left half sets .5, right half sets whole', async ({ page }) => {
  const handle = await loadComponent(
    page,
    {
      template: `<ngn-rating [step]="0.5" [value]="inputs().value" (valueChange)="output('value', $event)" />`,
      imports: ['rating'],
    },
    { inputs: { value: null } }
  );
...
test('disabled: reflects aria-disabled', async ({ page }) => {
  await loadComponent(
    page,
    {
      template: `<ngn-rating [value]="3" [disabled]="true" />`,
      imports: ['rating'],
    },
    { inputs: {} }
  );
  await expect(page.locator('ngn-rating')).toHaveAttribute('aria-disabled', 'true');
});
...
test('indicatorTemplate receives fillRatio and index, replaces default icons', async ({ page }) => {
  ...
  const indicators = page.locator('ngn-rating .ind');
  ...
  await expect(page.locator('ngn-rating ngn-icon')).toHaveCount(0);
});
...
test('content-child #indicator takes precedence over the indicatorTemplate input', async ({
  ...
  await loadComponent(
    page,
    {
      template: `
        ...
        <ngn-rating [value]="2.5" [indicatorTemplate]="alt">
  ...
  await expect(page.locator('ngn-rating .ind')).toHaveCount(5);
  await expect(page.locator('ngn-rating .alt-ind')).toHaveCount(0);
});
