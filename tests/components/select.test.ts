...
test('editable: field padding belongs to the input', async ({ page }) => {
  await loadComponent(
    ...
    {
      template: `
      <awd-input-field style="width: 300px;">
        <awd-select [options]="inputs().options" [editable]="true" />
      </awd-input-field>
    `,
  ...
  const box = (await page.locator('awd-input-field > div').first().boundingBox())!;
  const midY = box.y + box.height / 2;
  const caret = () => input.evaluate((el: HTMLInputElement) => el.selectionStart);

  // The input is nested two levels down (awd-select > .select-input > input) and
...
test('base', async ({ page }, testInfo) => {
  const handle = await loadComponent(
    ...
    {
      template: `
      <awd-input-field style="width: 200px;">
        <awd-select [options]="inputs().options" [popoverOptions]="inputs().popoverOptions" />
      </awd-input-field>
    `,
      imports: ['select', 'inputField'],
    },
  ...
  const select = new NgnSelectHarness(page.locator('awd-select').first());
...
test('invalid', async ({ page }, testInfo) => {
  const handle = await loadComponent(
    page,
    {
      template: `
      <awd-input-field style="width: 200px;">
        <awd-select [invalid]="inputs().invalid" [touched]="inputs().touched" [options]="inputs().options" [popoverOptions]="inputs().popoverOptions" />
      </awd-input-field>
    `,
      imports: ['select', 'inputField'],
    },
...
test('opens from the wrapping field padding with tabindex -1', async ({ page }) => {
  await loadComponent(
    page,
    {
      template: `
      <awd-input-field style="width: 200px;">
        <awd-select [tabindex]="-1" [options]="inputs().options" />
      </awd-input-field>
    `,
      imports: ['select', 'inputField'],
    },
  ...
  const select = new NgnSelectHarness(page.locator('awd-select').first());
  const field = page.locator('awd-input-field > div').first();
  ...
  expect(await page.evaluate(() => document.activeElement?.closest('awd-select') !== null)).toBe(
    true
  );
});
...
test('filter input keeps Home and End for the caret', async ({ page }) => {
  await loadComponent(
    page,
    {
      template: `
      <awd-input-field style="width: 200px;">
        <awd-select [options]="inputs().options" [filter]="true" />
      </awd-input-field>
    `,
      imports: ['select', 'inputField'],
    },
