...
test('renders an inlined, aria-hidden svg for an Iconify icon', async ({ page }) => {
  await loadComponent(
    page,
    { template: `<ngn-icon [icon]="inputs().icon" />`, imports: ['icon'] },
    { inputs: { icon: ICONIFY } }
  );

  const host = page.locator('ngn-icon');
  await expect(host).toHaveClass(/ngn-icon-root/);
...
test('applies scale by insetting the viewBox', async ({ page }) => {
  ...
  await expect(page.locator('ngn-icon svg')).toHaveAttribute('viewBox', '6 6 12 12');
});

test('resolves a registered default icon', async ({ page }) => {
  await loadComponent(
    page,
    { template: `<ngn-icon [defaultIcon]="'checkbox-checked'" />`, imports: ['icon'] },
    { inputs: {} }
  );

  const svg = page.locator('ngn-icon svg');
  await expect(svg).toHaveCount(1);
  await expect(svg).toHaveAttribute('aria-hidden', 'true');
});
...
test('icon takes precedence over defaultIcon', async ({ page }) => {
  await loadComponent(
    page,
    {
      template: `<ngn-icon [icon]="inputs().icon" [defaultIcon]="'checkbox-checked'" />`,
      imports: ['icon'],
    },
    { inputs: { icon: ICONIFY } }
  );
  ...
  await expect(page.locator('ngn-icon svg')).toHaveAttribute('viewBox', '0 0 24 24');
});
