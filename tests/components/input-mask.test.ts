...
async function loadTimeMask(page: any) {
  const handle = await loadComponent(
    page,
    {
      imports: ['inputField', 'inputMask'],
      template: `<ngn-input-field>
        <ngn-input-mask [mask]="inputs().mask" (valueChange)="output('value', $event)" />
      </ngn-input-field>`,
    },
    {
      inputs: { mask: 'time' },
...
test('paste applies a valid time string', async ({ page }) => {
  ...
  const simulatePaste = (text: string) =>
    page.evaluate((t: string) => {
      const input = document.querySelector('ngn-input-mask input') as HTMLInputElement;
      input.focus();
      input.dispatchEvent(
...
test('active-section highlight only shows while focused', async ({ page }, testInfo) => {
  ...
  // The class selector for section-active elements (e.g. '.ngn-input-mask-section-active')
...
// `NgnInputMask` focuses the proxy and calls `setActive` for the section whose
...
test('clicking input-field padding selects nearest section by horizontal position', async ({
  ...
  await loadComponent(
    ...
    {
      imports: ['inputField', 'inputMask'],
      template: `<ngn-input-field><ngn-input-mask [mask]="inputs().mask" /></ngn-input-field>`,
    },
    { inputs: { mask: 'time' } }
  );
  const mask = new NgnInputMaskHarness(page.locator('ngn-input-mask').first());
  ...
  // Locate the field root div (not the ngn-input-field host, but the inner
  // wrapper that receives the (click) handler). It wraps all content and has
  // padding around the mask sections.
  const fieldRoot = page.locator('ngn-input-field').first();
