  ...
  const handle = await loadComponent(
    page,
    {
      ...
      template: `<ngn-input-field>
        <ngn-mask-input [mask]="inputs().mask" (valueChange)="output('value', $event)" />
      </ngn-input-field>`,
    },
    {
      inputs: { mask: 'time' },
...
test('paste applies a valid time string', async ({ page }) => {
  ...
  const simulatePaste = (text: string) =>
    page.evaluate((t: string) => {
      const input = document.querySelector('ngn-mask-input input') as HTMLInputElement;
      input.focus();
      input.dispatchEvent(
...
test('active-section highlight only shows while focused', async ({ page }, testInfo) => {
  ...
  // The class selector for section-active elements (e.g. '.ngn-mask-input-section-active')
...
test('clicking input-field padding selects nearest section by horizontal position', async ({
  ...
  await loadComponent(
    ...
    {
      imports: ['inputField', 'maskInput'],
      template: `<ngn-input-field><ngn-mask-input [mask]="inputs().mask" /></ngn-input-field>`,
    },
    { inputs: { mask: 'time' } }
  );
  const mask = new NgnMaskInputHarness(page.locator('ngn-mask-input').first());
